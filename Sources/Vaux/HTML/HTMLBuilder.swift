...
@_functionBuilder public struct HTMLBuilder {
	...
	public static func buildIf(_ content: HTML?) -> HTML {
		return content ?? MultiNode(children: [])
	}
	
	/// If the condition of an `if` statement is `true`, then this method will be called, and the result of evaluating the expressions in the `true` block will be returned unmodified.
	/// - Note: We do not need to preserve type information from both the `true` and `false` blocks, so this function does not wrap its passed value.
	/// - Parameters:
