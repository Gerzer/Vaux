...
//
//
//  Created by Mathieu Barnachon on 6/28/2019.
//
...
struct AttributedSVGNode: SVG {
	...
	func getTag() -> String? {
		return self.child.getTag()
	}
	
	func renderAsHTML(into stream: HTMLOutputStream, attributes: [Attribute]) {
		var fullAttrs = attributes
		fullAttrs.append(self.attribute)
		self.child.renderAsHTML(into: stream, attributes: fullAttrs)
	}
...
public struct SVGFilter: SVG {
	...
	public init(id: String, types: [SVGFilterType], attributes: [Attribute] = []) {
		node = SVGNode(tag: "filter", child: MultiSVGNode(children: types.compactMap { (type) -> SVG? in
			return type.node
		}))
	...
	public func renderAsHTML(into stream: HTMLOutputStream, attributes: [Attribute]) {
		self.node.renderAsHTML(into: stream, attributes: self.attributes + attributes)
	}
...
public struct SVGGradient: SVG {
	...
	public init(id: String, type: GradientType, offsets: [(offset: String, style: [StyleAttribute])], attributes: [Attribute] = []) {
		let stops = offsets.compactMap { (offsetEntry) -> SVG? in
			SVGNode(tag: "stop")
				.attr("offset", offsetEntry.offset)
				.style(offsetEntry.style)
		}
		switch type {
		case .linear:
			self.node = SVGNode(tag: "linearGradient", child: MultiSVGNode(children: stops))
		case .radial:
			self.node = SVGNode(tag: "radialGradient", child: MultiSVGNode(children: stops))
		}
		self.attributes = [
	...
	public init(id: String, controls: (x1: String, y1: String, x2: String, y2: String), offsets: [(offset: String, style: [StyleAttribute])]) {
		let stops = offsets.compactMap { (offsetEntry) -> SVG? in
			SVGNode(tag: "stop")
				.attr("offset", offsetEntry.offset)
				.style(offsetEntry.style)
	...
	public init(id: String, controls: (cx: String, cy: String, r: String, fx: String, fy: String), offsets: [(offset: String, style: [StyleAttribute])]) {
		let stops = offsets.compactMap { (offsetEntry) -> SVG? in
			SVGNode(tag: "stop")
				.attr("offset", offsetEntry.offset)
				.style(offsetEntry.style)
